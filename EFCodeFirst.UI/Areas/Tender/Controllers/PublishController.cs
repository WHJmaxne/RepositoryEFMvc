...
namespace EFCodeFirst.UI.Areas.Tender.Controllers
{
    public class PublishController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            {
                list = operContext.BllSession.TApplyBLL.LoadPageEntities(pageIndex, pageSize, out totalCount, out totalPage, u => u.IsDelete == false && u.ProjectManager == operContext.UserSession.Id && u.ApplyState == ApplyState.PendingRelease && (u.ApplyName.Contains(seachValue) || u.ApplyId.Contains(seachValue)), u => u.Id, false, "UserInfo", "BillType").ToList();
            }
            else
            {
                list = operContext.BllSession.TApplyBLL.LoadPageEntities(pageIndex, pageSize, out totalCount, out totalPage, u => u.IsDelete == false && u.ProjectManager == operContext.UserSession.Id && u.ApplyState == ApplyState.PendingRelease, u => u.Id, false, "UserInfo", "BillType").ToList();
            }
        ...
        public ActionResult Publish()
        {
            try
            {
                ...
                tapply.ApplyState = ApplyState.Completed;
